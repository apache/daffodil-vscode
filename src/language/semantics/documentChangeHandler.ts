...
import {
  XMLConfiguration,
  XSLTLightConfiguration,
} from './languageConfigurations'
...
export class DocumentChangeHandler {
  ...
  public async onDocumentChange(
    ...
    if (activeChange.text === '/' && activeChange.rangeLength === 0) {
      ...
      if (nextChar === '>') {
        let textBeforeCloseEnd = this.lexer.getTextBeforeCloseEnd(
          e.document,
          activeChange
        )
        if (textBeforeCloseEnd) {
          let newStartChar =
            activeChange.range.start.character - textBeforeCloseEnd.length // positive number
          let activeStartPos = activeChange.range.start
          let newStartPos = new vscode.Position(
            activeStartPos.line,
            newStartChar + 1
          )
          let newRange = new vscode.Range(newStartPos, newStartPos)
          let newOffset =
            activeChange.rangeOffset - (textBeforeCloseEnd.length + 1)
          let endTagPosData = this.lexer.getEndTagForStartTagChange(
            e.document,
            newOffset,
            activeChange.range.start.line,
            newStartChar,
            newRange,
            true
          )
          if (endTagPosData && endTagPosData?.startTag.endsWith('/')) {
            endTagPosData.startTag = endTagPosData.startTag.substring(
              0,
              endTagPosData.startTag.length - 1
            )
          }
          if (
            endTagPosData &&
            endTagPosData.startTag === endTagPosData.endTag
          ) {
            let endLine = e.document.lineAt(
              endTagPosData.startPosition.line
            ).text
            let followingStartCloseTag = endLine.substring(
              endTagPosData.startPosition.character
            )
            let closeTagCharPos = followingStartCloseTag.indexOf('>')
            if (closeTagCharPos > -1) {
              let startPos = new vscode.Position(
                activeChange.range.start.line,
                activeChange.range.start.character + 1
              )
              let endCharPos =
                endTagPosData.startPosition.character + closeTagCharPos
    ...
    if (!skipTrigger && activeChange.rangeOffset > 10) {
      ...
      if (
        (prevChar === '"' || prevChar === '(') &&
        activeChange.text.length === 1 &&
        ['[', '(', '{', '?', '"', "'"].indexOf(activeChange.text) === -1
      ) {
        triggerSuggest = true
      }
    }
    ...
    if (
      ...
      !this.changesAreEqual(this.lastChangePerformed, activeChange)
    ) {
      let tagNameLengthBeforeEdit = this.lexer.isStartTagChange(
        e.document,
        activeChange
      )
      if (tagNameLengthBeforeEdit > -1) {
        let startBracketPos =
          activeChange.range.start.character - (tagNameLengthBeforeEdit + 1)
        let startTagOffset =
          activeChange.range.start.character - startBracketPos // positive number
        ...
        if (endTagPosData) {
          if (this.cachedFailedEdit) {
            ...
            let updateEndPos = new vscode.Position(
              startLline,
              startChar + endTagPosData.endTag.length
            )
            let updateRange = new vscode.Range(updateStartPos, updateEndPos)
            // replace the whole endtag to be safe:
            this.lastChangePerformed = {
              range: updateRange,
              text: endTagPosData.startTag,
              fullTagName: endTagPosData.startTag,
            }
            await this.performRename(
              e.document,
              Object.assign(this.lastChangePerformed)
            )
          }
          let endTagNameOk = this.checkEndTag(
            endTagPosData,
            tagNameLengthBeforeEdit,
            activeChange
          )
          if (endTagNameOk) {
            let endTagPos = endTagPosData.startPosition
            let adjustedStartTagPos =
              endTagPos.character + tagNameLengthBeforeEdit
            let updateStartPos = new vscode.Position(
              endTagPos.line,
              adjustedStartTagPos
            )
            let updateEndPos = new vscode.Position(
              endTagPos.line,
              adjustedStartTagPos + activeChange.rangeLength
            )
            let updateRange = new vscode.Range(updateStartPos, updateEndPos)
            this.lastChangePerformed = {
              range: updateRange,
              text: activeChange.text,
              fullTagName: endTagPosData.startTag,
            }
            await this.performRename(
              e.document,
  ...
  private checkEndTag(
    endTagData: TagRenamePosition,
    startTagPos: number,
    startTagChange: vscode.TextDocumentContentChangeEvent
  ): boolean {
    let result = false
    ...
    let originalStartTagLength =
      endTagData.startTag.length -
      (startTagChange.text.length - startTagChange.rangeLength)
    if (originalStartTagLength !== endTagData.endTag.length) {
      return false
    }
    let beforeEndTag = endTagName.substring(0, startTagPos)
    let afterEndTag = endTagName.substring(
      startTagPos + startTagChange.rangeLength
    )
  ...
  private registerXMLDocument = (editor: vscode.TextEditor) => {
    ...
    let isXMLDocument =
      ...
      document.languageId === 'xslt' ||
      document.languageId === 'dcp' ||
      document.languageId === 'sch'
    let isXPathDocument =
      document.languageId === 'xpath' || document.languageId === 'dfdl'

    if (
      this.xmlDocumentRegistered &&
      (!isXMLDocument || !isXPathDocument) &&
    ...
    if (isXMLDocument && !this.xmlDocumentRegistered) {
      ...
      this.onDidChangeRegistration = vscode.workspace.onDidChangeTextDocument(
        (e) => this.onDocumentChange(e, isXMLDocument)
      )
  ...
  private changesAreEqual(
    tagRenameEdit: TagRenameEdit,
    change2: vscode.TextDocumentContentChangeEvent
  ) {
  ...
  private static getLastDocXmlnsPrefixes() {
    if (DocumentChangeHandler.lastActiveXMLEditor) {
      DocumentChangeHandler.lastXMLDocumentGlobalData = this.lexer.analyseLight(
        DocumentChangeHandler.lastActiveXMLEditor.document.getText(),
        true
      )
    }
  }
