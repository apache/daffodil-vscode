...
export class Debug {
  ...
  public static printSerializedTokens(
    testTitle: string,
    testXpath: string,
    tokens: Token[]
  ) {
  ...
  private static serializeTokens = function (
    accumulator: any,
    token: Token | null
  ): any {
    if (!token) {
    ...
    let tokenType =
      'TokenLevelState.' + Debug.tokenStateToString(token.tokenType)
    let charType = 'CharLevelState.' + Debug.charStateToString(token.charType)
    let childrenString: string = ''
    if (token.children) {
      childrenString = ',\nchildren:'
      childrenString +=
        '[' + token.children.reduce(Debug.serializeTokens, '') + ']'
    }
    let objectString = `
{value: "${value}",
charType: ${charType},
tokenType: ${tokenType + childrenString}
  ...
  public static printMinSerializedTokens(
    testTitle: string,
    testXpath: string,
    tokens: Token[]
  ) {
  ...
  private static minSerializeTokens = function (
    accumulator: any,
    token: Token | null
  ): any {
    if (!token || XPathLexer.isCharTypeEqual(token, CharLevelState.lWs)) {
      ...
      if (token.children) {
        childrenString = ',\nchildren:'
        childrenString +=
          '[' + token.children.reduce(Debug.minSerializeTokens, '') + ']'
      }
      let objectString = `
    {value: \`${value}\`,${err}
    tokenType: ${tokenType + childrenString},
  ...
  private static showTokens = function (token: Token) {
    ...
    console.log(
      Debug.padString(tokenValue) +
        Debug.padString(charState) +
        Debug.tokenStateToString(token.tokenType) +
        err
    )
    if (token.children) {
  ...
  public static printDebugOutput(
    cachedRealToken: Token | null,
    newValue: Token,
    lineNumber: number,
    ...
    if (newValue.value !== '') {
      ...
      let cachedRealTokenString: string = cachedRealToken
        ? this.getTokenDebugString(cachedRealToken)
        : ''
      ...
      if (newValue.charType === CharLevelState.lWs && !showWhitespace) {
        ...
        console.log(
          cachedRealTokenString + cachedTpadding + newT + posPadding + posString
        )
      }
    }
  }

  public static printStateOuput(
    prevRealToken: Token,
    currentLabelState: CharLevelState,
    nextLabelState: CharLevelState,
    token: string
  ) {
    ...
    console.log('to:   ' + Debug.charStateToString(nextLabelState)) +
      '[' +
      token +
      ']'
  }
  ...
  public static getTokenDebugString(lrt: Token) {
    ...
    let oldT: string =
      prevType + this.padParts(prevTypeLength) + prevToken + '_'
    return oldT
  }
  ...
  public static charStateToString(
    stringCommentState: CharLevelState | undefined
  ): string {
