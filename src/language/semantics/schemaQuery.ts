...
export class SchemaQuery {
  ...
  public getExpected(name: string, attributeName?: string) {
    ...
    if (this.schema.docType === DocumentTypes.XSLT && !isXsltName) {
      let attGroup =
        this.schema.attributeGroups['xsl:literal-result-element-attributes']
      if (attGroup.attrs) {
    ...
    if (this.schema.docType === DocumentTypes.SCH && !isXsltName) {
      let attGroupName = this.schema.elements[name]
        ? this.schema.elements[name].attributeGroup
        : undefined
      if (attGroupName) {
    ...
    if (!ct && this.schema.substitutionGroups) {
      ...
      let sgElement = <ComplexType>(
        this.schema.substitutionGroups.instruction.elements[name]
      )
      if (!sgElement) {
        isInstructionSg = false
        sgElement = <ComplexType>(
          this.schema.substitutionGroups.declaration.elements[name]
        )
      }
      if (!sgElement) {
        sgElement = <ComplexType>(
          this.schema.substitutionGroups.ixslInstruction.elements[name]
        )
      }
      if (sgElement) {
  ...
  private lookupSimpleType(sgType: SimpleType, result: Expected) {
    if (sgType && sgType.enum) {
      sgType.enum.forEach((attrValue) => {
        ...
        let existing = result.attributeValues.find(
          (val) => val[0] === attrValue
        )
        if (!existing) {
  ...
  private performSubstitutions(elements: [string, string][]) {
    ...
    elements.forEach((item) => {
      if (item[0] === 'xsl:instruction' && this.schema.substitutionGroups) {
        let subElements = Object.keys(
          this.schema.substitutionGroups.instruction.elements
        )
        ...
        if (this.useIxsl) {
          let subElements = Object.keys(
            this.schema.substitutionGroups.ixslInstruction.elements
          )
          subElements.forEach((se) => {
            newElements.push([se, ''])
          })
        ...
        item[0] === 'xsl:declaration' &&
        this.schema.substitutionGroups
      ) {
        let subElements = Object.keys(
          this.schema.substitutionGroups.declaration.elements
        )
        subElements.forEach((se) => {
          newElements.push([se, ''])
        })
