...
export class XsltTokenDiagnostics {
        ...
        if (elementStack?.length === 0) {
          ...
          const expectedNames: string[] =
            elementStack && elementStack.length > 0
              ...
              : []
          valid =
            expectedNames.indexOf('sch:' + name) > -1
              ? NameValidationError.None
              : NameValidationError.XSLTElementNameError
        }
      } else if (
        (type === ValidationType.XSLTAttribute ||
          (isSchematron && type === ValidationType.XMLAttribute)) &&
        expectedAttributes
      ) {
        valid =
  ...
  public static calculateDiagnostics = (
    ...
    if (isSchematron && XSLTConfiguration.configuration.schemaData) {
      xsltSchemaQuery = new SchemaQuery(
        XSLTConfiguration.configuration.schemaData
      )
    }
    ...
    allTokens.forEach((token, index) => {
      ...
      let isXMLToken =
        token.tokenType >= XsltTokenDiagnostics.xsltStartTokenNumber
      if (isXMLToken) {
        ...
        let xmlTokenType = <XSLTokenLevelState>(
          (token.tokenType - XsltTokenDiagnostics.xsltStartTokenNumber)
        )
        ...
        switch (xmlTokenType) {
          case XSLTokenLevelState.xmlText:
            if (elementStack.length === 0 && token.startCharacter > -1) {
              const tValue = XsltTokenDiagnostics.getTextForToken(
                lineNumber,
                token,
                document
              )
          ...
          case XSLTokenLevelState.xslElementName:
            ...
            tagElementName = XsltTokenDiagnostics.getTextForToken(
              lineNumber,
              token,
              document
            )
            ...
            const lookupElementName =
              isSchematron && !isXsltElementName && !isSchElementName
                ? 'sch:' + tagElementName
                : tagElementName
            const realSchemaQuery =
              xsltSchemaQuery && tagElementName.startsWith('xsl:')
                ? xsltSchemaQuery
                : schemaQuery

            ;[tagElementChildren, tagElementAttributes] =
              XsltTokenDiagnostics.getExpectedElementNames(
                lookupElementName,
                realSchemaQuery,
                elementStack
              )
            ...
            if (tagType === TagType.Start) {
              ...
              tagType =
                xslVariable.indexOf(tagElementName) > -1
                  ? TagType.XSLTvar
                  : TagType.XSLTstart
              ...
              if (
                !includeOrImport &&
                tagType !== TagType.XSLTvar &&
                elementStack.length === 1
              ) {
                includeOrImport =
                  tagElementName === XsltTokenDiagnostics.xslImport ||
                  tagElementName === XsltTokenDiagnostics.xslInclude
              }
            ...
            switch (xmlCharType) {
              ...
              case XMLCharState.rSelfCtNoAtt:
                ...
                if (tagExcludeResultPrefixes) {
                  ...
                  if (
                    !(
                      tagExcludeResultPrefixes.prefixes.length === 1 &&
                      tagExcludeResultPrefixes.prefixes[0] === '#all'
                    )
                  ) {
                    missingPrefix = tagExcludeResultPrefixes.prefixes.find(
                      (pfx) => {
                        if (
                          pfx !== '#default' &&
                          inheritedPrefixes.indexOf(pfx) < 0
                        )
                          return pfx
                      }
                    )
                ...
                const attrValType = tagElementName.startsWith('xsl:')
                  ? ValidationType.XSLTAttribute
                  : ValidationType.XMLAttribute

                let tunnelAttributeFound = false
                const checkPendingErrors =
                  pendingTemplateParamErrors.length !== 0
                tagAttributeNames.forEach((attName) => {
                  ...
                  if (validateResult === NameValidationError.NameError) {
                    ...
                    validateResult === NameValidationError.NamespaceError
                  ) {
                ...
                if (checkPendingErrors && !tunnelAttributeFound) {
                  pendingTemplateParamErrors.forEach((item) =>
                    problemTokens.push(item)
                  )
                }
                pendingTemplateParamErrors = []

                if (startTagToken && !problem) {
                  let validationError = XsltTokenDiagnostics.validateName(
                    tagElementName,
                    ValidationType.XMLElement,
                    isSchematron,
                    inheritedPrefixes,
                    elementStack
                  )
                  if (validationError !== NameValidationError.None) {
                    startTagToken['error'] =
                      validationError === NameValidationError.NameError
                        ? ErrorType.XMLName
                        : validationError === NameValidationError.NamespaceError
                        ? ErrorType.XMLXMLNS
                        : ErrorType.XSLTInstrUnexpected
                    startTagToken['value'] = tagElementName
                    problemTokens.push(startTagToken)
                  } else if (attsWithNameErrors.length > 0) {
                    startTagToken['error'] = ErrorType.XMLAttributeName
                    startTagToken['value'] =
                      tagElementName + "': '" + attsWithNameErrors.join("', ")
                    problemTokens.push(startTagToken)
                  } else if (attsWithXmlnsErrors.length > 0) {
                    startTagToken['error'] = ErrorType.XMLAttributeXMLNS
                    startTagToken['value'] =
                      tagElementName + "': '" + attsWithXmlnsErrors.join("', ")
                    problemTokens.push(startTagToken)
                    ...
                    startTagToken['value'] =
                      tagElementName +
                      "': '" +
                      xsltAttsWithNameErrors.join("', ")
                    problemTokens.push(startTagToken)
                  }
                }

                if (
                  xmlCharType === XMLCharState.rStNoAtt ||
                  xmlCharType === XMLCharState.rSt
                ) {
                  // on a start tag
                  if (tagElementName === 'xsl:accumulator') {
                    inScopeVariablesList.push({ token: token, name: 'value' })
                  } else if (tagElementName === 'xsl:catch') {
                    XsltTokenDiagnostics.xsltCatchVariables.forEach(
                      (catchVar) => {
                        inScopeVariablesList.push({
                          token: token,
                          name: catchVar,
                        })
                      }
                  ...
                  const stackElementChildren = isSchematron
                    ? tagElementChildren
                    : attrValType === ValidationType.XMLAttribute &&
                      elementStack.length > 0
                  ...
                  const childSymbols: vscode.DocumentSymbol[] =
                    XsltTokenDiagnostics.initChildrenSymbols(
                      tagAttributeSymbols
                    )
                  ...
                  if (variableData !== null) {
                    ...
                    if (startTagToken) {
                      ...
                      elementStack.push({
                        namespacePrefixes: inheritedPrefixesCopy,
                        currentVariable: variableData,
                        variables: newVariablesList,
                        symbolName: tagElementName,
                        symbolID: tagIdentifierName,
                        identifierToken: startTagToken,
                        childSymbols: childSymbols,
                        expectedChildElements: stackElementChildren,
                      })
                    }
                  ...
                  if (startTagToken) {
                    let symbol =
                      XsltTokenDiagnostics.createSymbolFromElementTokens(
                        tagElementName,
                        tagIdentifierName,
                        startTagToken,
                        token
              ...
              case XMLCharState.rCt:
                ...
                if (elementStack.length > 0) {
                  ...
                  if (
                    tagElementName === 'xsl:iterate' &&
                    currentXSLTIterateParams.length > 0
                  ) {
                    currentXSLTIterateParams.pop()
                  }
                  if (poppedData) {
                    if (poppedData.symbolName !== tagElementName) {
                      let errorToken = Object.assign(
                        {},
                  ...
                  if (poppedData) {
                    ...
                    inScopeVariablesList = poppedData
                      ? poppedData.variables
                      : []
                    if (poppedData.currentVariable) {
                      if (docType === DocumentTypes.DCP) {
                        importedGlobalVarNames.push(
                          poppedData.currentVariable.name
                        )
          ...
          case XSLTokenLevelState.xmlnsName:
            rootXmlnsName = null
            let attNameText = XsltTokenDiagnostics.getTextForToken(
              lineNumber,
              token,
              document
            )
            ...
            if (
              !problemReported &&
              token.charType === XMLCharState.syntaxError &&
              prevToken &&
              !prevToken.error
            ) {
              problemReported = true
              prevToken['error'] = ErrorType.XMLAttEqualExpected
              prevToken['value'] = XsltTokenDiagnostics.getTextForToken(
                prevToken.line,
                prevToken,
                document
              )
            ...
            if (!problemReported) {
              if (xmlTokenType === XSLTokenLevelState.xmlnsName) {
                ...
                tagAttributeSymbols.push(
                  XsltTokenDiagnostics.createSymbolForAttribute(
                    token,
                    attNameText
                  )
                )
            ...
            if (tagType === TagType.XSLTvar) {
              ...
              if (
                ...
                attNameText === XsltTokenDiagnostics.excludePrefixes ||
                attNameText === XsltTokenDiagnostics.xslExcludePrefixes
              ) {
          ...
          case XSLTokenLevelState.attributeValue:
            let fullVariableName = XsltTokenDiagnostics.getTextForToken(
              lineNumber,
              token,
              document
            )
            if (tagAttributeSymbols.length > 0) {
              if (fullVariableName.length !== 1) {
                tagAttributeSymbols[tagAttributeSymbols.length - 1].detail =
                  fullVariableName
              } else {
                tagAttributeSymbols[tagAttributeSymbols.length - 1].kind =
                  vscode.SymbolKind.Event
              }
            }
            let variableName = fullVariableName.substring(
              1,
              fullVariableName.length - 1
            )
            let hasProblem = false
            if (rootXmlnsName !== null) {
              let prefix =
                rootXmlnsName.length === 5 ? '' : rootXmlnsName.substr(6)
            ...
            switch (attType) {
              ...
              case AttributeType.ExcludeResultPrefixes:
                ...
                tagExcludeResultPrefixes = {
                  token: token,
                  prefixes: excludePrefixes,
                }
                break
              case AttributeType.None:
                if (
                  ...
                  prevToken.length === 1 &&
                  prevToken.tokenType ===
                    XsltTokenDiagnostics.xsltStartTokenNumber +
            ...
            if (!hasProblem && attType === AttributeType.UseAttributeSets) {
              if (
                globalAttributeSetNames.indexOf(variableName) < 0 &&
                variableName !== 'xsl:original'
              ) {
            ...
            if (
              !hasProblem &&
              attType === AttributeType.InstructionMode &&
              tagElementName === 'xsl:apply-templates'
            ) {
            ...
            if (
              ...
              attType === AttributeType.InstructionName &&
              elementStack.length > 0 &&
              tagElementName === 'xsl:with-param'
            ) {
              let callTemplateName =
                elementStack[elementStack.length - 1].symbolID
              ...
              if (templateParams) {
                ...
                const params =
                  currentXSLTIterateParams[currentXSLTIterateParams.length - 1]
            ...
            if (
              (!hasProblem && attType === AttributeType.Variable) ||
              attType === AttributeType.InstructionName
            ) {
              if (!fullVariableName.includes('{')) {
                let vType = tagElementName.endsWith(':attribute')
                  ? ValidationType.XMLAttribute
                  : ValidationType.PrefixedName
                let validateResult = XsltTokenDiagnostics.validateName(
                  variableName,
                  vType,
                  isSchematron,
          ...
          case XSLTokenLevelState.processingInstrName:
            let piName = XsltTokenDiagnostics.getTextForToken(
              lineNumber,
              token,
              document
            )
          ...
          case XSLTokenLevelState.entityRef:
            let entityName = XsltTokenDiagnostics.getTextForToken(
              lineNumber,
              token,
              document
            )
          ...
          case XSLTokenLevelState.processingInstrValue:
            if (isXMLDeclaration) {
              XsltTokenDiagnostics.validateXMLDeclaration(
                lineNumber,
                token,
                document,
                problemTokens
          ...
          case XSLTokenLevelState.dtdEnd:
            if (dtdEnded) {
              let endDtd = XsltTokenDiagnostics.getTextForToken(
                lineNumber,
                token,
                document
              )
          ...
          case XSLTokenLevelState.dtd:
            if (onRootStartTag && !dtdEnded) {
              ...
              let dtdValue = XsltTokenDiagnostics.getTextForToken(
                lineNumber,
                token,
                document
              )
        ...
        switch (xpathTokenType) {
          ...
          case TokenLevelState.operator:
            ...
            if (prevToken?.tokenType === TokenLevelState.complexExpression) {
              ...
              let isXMLToken =
                prevToken.tokenType >= XsltTokenDiagnostics.xsltStartTokenNumber
              let currCharType = <CharLevelState>token.charType
              let nextToken =
  ...
  public static getExpectedElementNames(
    parentName: string,
    ...
    if (
      (parentName.startsWith('xsl') &&
        schemaQuery &&
        schemaQuery.docType === DocumentTypes.XSLT) ||
      (parentName.startsWith('sch') &&
        schemaQuery &&
        schemaQuery.docType === DocumentTypes.SCH)
    ) {
  ...
  private static validateXMLDeclaration(
    ...
    let piValue = XsltTokenDiagnostics.getTextForToken(
      lineNumber,
      token,
      document
    )
    ...
    allParts.forEach(function (part, index) {
          ...
          case XMLPIState.none:
            if (!spaceRegx.test(part)) {
              if (part === 'version' && names.indexOf(XMLPIName.version) < 0) {
                ...
                part === 'encoding' &&
                names.indexOf(XMLPIName.encoding) < 0
              ) {
                pState = XMLPIState.Name
                ...
                part === 'standalone' &&
                names.indexOf(XMLPIName.standalone) < 0
              ) {
                pState = XMLPIState.Name
      ...
      if (index === lastPartIndex) {
        if (pState === XMLPIState.invalid) {
          ...
          isValid =
            names.indexOf(XMLPIName.version) > -1 &&
            namesWithValues.indexOf(XMLPIName.version) > -1 &&
            names.indexOf(XMLPIName.encoding) > -1 ===
              namesWithValues.indexOf(XMLPIName.encoding) > -1 &&
            names.indexOf(XMLPIName.standalone) > -1 ===
              namesWithValues.indexOf(XMLPIName.standalone) > -1
        }
      }
    })
  ...
  public static getXPathVariableDefnToken(
    globalVarName: string | null,
    document: vscode.TextDocument,
    importedVariables: string[],
    token: BaseToken,
    ...
    result = this.resolveVariableName(
      inScopeXPathVariablesList,
      varName,
      xpathVariableCurrentlyBeingDefined,
      globalVariable
  ...
  public static createSymbolForAttribute(
    innerToken: BaseToken,
    attrName: string
  ) {
  ...
  public static initChildrenSymbols(attrSymbols: vscode.DocumentSymbol[]) {
    ...
    const startPos = new vscode.Position(
      attrSymbols[0].range.start.line,
      attrSymbols[0].range.start.character
    )
    const endPos = new vscode.Position(
      attrSymbols[attrSymbols.length - 1].range.end.line,
      attrSymbols[attrSymbols.length - 1].range.end.character
    )
    const range = new vscode.Range(startPos, endPos)
    const detail = ''
  ...
  private static getDiagnosticsFromUnusedVariableTokens(
    ...
    for (let token of unresolvedVariableTokens) {
      result.push(
        this.createUnresolvedVarDiagnostic(document, token, includeOrImport)
      )
    }
    return result
  }
  ...
  public static createImportDiagnostic(
