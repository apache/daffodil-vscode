...
export class XSLTSchema implements SchemaData {
  ...
  simpleTypes: { [name: string]: SimpleType } = {
    ...
    'xsl:component-kind-type': {
      base: ['xs:token'],
      enum: ['template', 'function', 'variable', 'attribute-set', 'mode', '*'],
    },
    ...
    'xsl:method': {
      base: ['xs:token', 'xsl:EQName'],
      enum: ['xml', 'xhtml', 'html', 'text', 'adaptive', 'json'],
    },
    ...
    'xsl:validation-strip-or-preserve': {
      base: ['xsl:validation-type'],
      enum: ['preserve', 'strip'],
    },
    'xsl:validation-type': {
      base: ['xs:token'],
      enum: ['strict', 'lax', 'preserve', 'strip'],
    },
    'xsl:visibility-type': {
      base: ['xs:token'],
      enum: ['public', 'private', 'final', 'abstract', 'hidden'],
    },
    'xsl:visibility-not-hidden-type': {
      base: ['xsl:visibility-type'],
      enum: ['public', 'private', 'final', 'abstract'],
    },
    'xsl:yes-or-no': {
      base: ['xs:token'],
      enum: ['yes', 'no', 'true', 'false', '1', '0'],
    },
    'xsl:yes-or-no-or-maybe': {
      base: ['xs:token'],
      enum: ['yes', 'no', 'true', 'false', '1', '0', 'maybe'],
    },
    'xsl:yes-or-no-or-omit': {
      base: ['xs:token'],
      enum: ['yes', 'no', 'true', 'false', '1', '0', 'omit'],
    },
    ...
    declaration: {
      ...
      elements: {
        ...
        'xsl:global-context-item': {
          ...
          attributeList: [
            { name: 'use', enum: ['required', 'optional', 'absent'] },
          ],
        },
        'xsl:import': {
          base: 'xsl:element-only-versioned-element-type',
        ...
        'xsl:mode': {
          ...
          attributeList: [
            { name: 'visibility', enum: ['public', 'private', 'final'] },
          ],
        },
        'xsl:namespace-alias': {
          base: 'xsl:element-only-versioned-element-type',
