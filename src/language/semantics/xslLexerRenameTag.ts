...
export class XslLexerRenameTag extends XslLexer {
  ...
  public isStartTagChange(
    ...
    let isValid =
      renameRange.end.line === renameRange.start.line &&
      !this.nonNameRgx.test(change.text)
    if (!isValid) {
      return -1
    }
  ...
  public getEndTagForStartTagChange(
    document: vscode.TextDocument,
    offset: number,
    line: number,
    character: number,
    renameRange: vscode.Range,
    ignoreFirstSelfCloseTag: boolean
  ): TagRenamePosition | null {
    ...
    while (lCharCount < xslLength + 1) {
      ...
      if (currentChar) {
        ...
        if (
          ignoreSelfCloseTag &&
          (nextState === XMLCharState.rSelfCtNoAtt ||
            nextState === XMLCharState.rSelfCt)
        ) {
        ...
        if (nextState === currentState) {
          ...
          switch (nextState) {
            ...
            case XMLCharState.lSt:
              ...
              breakLoop =
                (storeToken && lineNumberChar > renameStartChar) ||
                (!gotRenameName && lineNumber > renameLine)
              break
                  ...
                  if (gotRenameName && xmlElementStack === renameStackLength) {
                    ...
                    endTagStartPos = {
                      ...
                      startPosition: new vscode.Position(
                        lineNumber,
                        lineNumberChar - (tokenChars.length + 1)
                      ),
                    }
                    breakLoop = true
                  }
