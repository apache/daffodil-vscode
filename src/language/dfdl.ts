...
export function activate(context: vscode.ExtensionContext) {
	...
	const elementCompletionProvider = vscode.languages.registerCompletionItemProvider('dfdl', {
		...
		provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
			...
			const dfdlDefineVariable = new vscode.CompletionItem('dfdl:defineVariable');
			dfdlDefineVariable.insertText = new vscode.SnippetString('<xs:annotation>\n\t<xs:appinfo source="http://www.ogf.org/dfdl/">\n\t\t<dfdl:defineVariable name="$1"$0')
			...
			dfdlSetVariable.insertText = new vscode.SnippetString('<xs:annotation>\n\t<xs:appinfo source="http://www.ogf.org/dfdl/">\n\t\t<dfdl:setVariable ref=\"${1|' + definedVariables + '|}\"$0');
			...
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				...
				if (!checkBraceOpen(document, position) && !wholeLine.includes('assert') && !nearestOpenItem.includes('none')) {
					...
					if (wholeLine.includes('defineVariable')) {
						...
						xmlExternal.insertText = new vscode.SnippetString(preVal + 'external="${1|true,false|}"$0');
						xmlExternal.documentation = new vscode.MarkdownString("");

						const xmlDefaultValue = new vscode.CompletionItem('defaultValue=');
						xmlDefaultValue.insertText = new vscode.SnippetString(preVal + 'defaultValue="0$1"$0');
	...
	function checkLastItemOpen(document: vscode.TextDocument, position: vscode.Position) {
		...
		if ((previousLine.includes('</') || previousLine.includes('/>')) ||
			((wholeLine.includes('element') || wholeLine.includes('sequence') || wholeLine.includes('group') ||
				wholeLine.includes('Variable')) && (wholeLine.includes('</') || wholeLine.includes('/>')))) {
	...
	function nearestOpen(document: vscode.TextDocument, position: vscode.Position) {
		...
		while (lineNum !== -1) {
			...
			else if (wholeLine.includes('defineVariable') && !wholeLine.includes('/>')) {
	...
	function checkDefineVariableOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
		while (lineNum !== 0) {
			const wholeLine = document.lineAt(lineNum).text.substr(0, document.lineAt(lineNum).range.end.character);
			if (wholeLine.includes('<dfdl:defineVariable') && (!wholeLine.includes("</dfdl:defineVariable")) && (!wholeLine.includes('/>'))) {
				return true;
			}
			--lineNum;
		}
	...
	function checkSetVariableOpen(document: vscode.TextDocument, position: vscode.Position) {
		var lineNum = position.line;
		while (lineNum !== 0) {
			const wholeLine = document.lineAt(lineNum).text.substr(0, document.lineAt(lineNum).range.end.character);
			if (wholeLine.includes('<dfdl:setVariable') && (!wholeLine.includes("</dfdl:setVariable")) && (!wholeLine.includes('/>'))) {
				return true;
			}
			--lineNum;
		}
		...
		'dfdl',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				...
				if (wholeLine.endsWith('>') && (wholeLine.includes('xs:element') || nearestOpenItem.includes('element')
					...
					|| wholeLine.includes('dfdl:defineVariable') || nearestOpenItem.includes('Variable')
				)) {
...
//					var backspace = new vscode.Selection(backpos, backpos);
					...
					if (wholeLine.endsWith('>') && (wholeLine.includes('xs:element ref') || wholeLine.includes('xs:group ref'))) {
					...
					else if (wholeLine.endsWith('>') && (wholeLine.includes('dfdl:defineVariable')) || (nearestOpenItem.includes('defineVariable'))) {
						...
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString('</xs:appinfo>\n'), backpos2);
						var backpos3 = position.with(position.line+3, startPos - 4);
						vscode.window.activeTextEditor?.insertSnippet(new vscode.SnippetString('</xs:annotation>$0'), backpos3);

					}
					else if (wholeLine.endsWith('>') && (wholeLine.includes('dfdl:setVariable')) || (nearestOpenItem.includes('setVariable'))) {
						var startPos = document.lineAt(position).text.indexOf('<', 0);
						var range = new vscode.Range(backpos, position);
						vscode.window.activeTextEditor?.edit(editBuilder => {
							editBuilder.replace(range, '');
...
//					var backspace = new vscode.Selection(backpos, backpos);
				...
				if (wholeLine.endsWith('/') && (wholeLine.includes('dfdl:defineVariable') || wholeLine.includes('dfdl:setVariable')
					|| nearestOpenItem.includes('defineVariable') || nearestOpenItem.includes('setVariable'))) {
					var startPos = document.lineAt(position).text.indexOf('<', 0);
					var range = new vscode.Range(backpos, position);
					vscode.window.activeTextEditor?.edit(editBuilder => {
						editBuilder.replace(range, '');
