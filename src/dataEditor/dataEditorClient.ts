...
async function createDataEditorWebviewPanel(
  ...
  const normalizeDataEditorPath = (file: string) => {
    file = file.replace(editor_config.WorkspaceKeyword, editor_config.rootPath)
    file = path.resolve(file)
    return file
  }
  //prompt file prompt first.
  const fileToEdit = await fileProvider
    .getFile(normalizeDataEditorPath)
    .then(normalizeDataEditorPath)
    .catch((err) => {
      throw new Error(err)
    })
