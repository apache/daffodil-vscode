...
export async function activate(ctx: vscode.ExtensionContext) {
    ...
    ctx.subscriptions.push(vscode.debug.onDidTerminateDebugSession((s) => {
        sid = undefined;
    }));
    ctx.subscriptions.push(vscode.debug.onDidReceiveDebugSessionCustomEvent(handleEvent));    
    ctx.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider('daffodil:infoset', fileInfosetProvider));

    ctx.subscriptions.push(vscode.commands.registerCommand('infoset.display', async () => {
        if(sid !== undefined) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('infoset.save', async (id: string, e: daf.InfosetEvent) => {
        if(sid !== undefined) {
...
const fileInfosetProvider = new class implements vscode.TextDocumentContentProvider {
