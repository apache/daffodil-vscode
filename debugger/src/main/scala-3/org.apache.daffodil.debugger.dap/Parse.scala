              ...
              IO.interruptibleMany(
                ...
                if (res.isError()) {
                  dapEvents
                    .send(
                      Parse.Event.Error(
                        res.getDiagnostics.asScala.toList
                          .map(d => d.toString)
                          .mkString("\n")
                      )
                    )
                ...
                new Parse.Exception(res.getDiagnostics.asScala.toList)
              }(res => !res.isError())
                .flatMap { parseResult =>
    ...
    case class StartElement(
      ...
      def this(pstate: PState, infoset: Option[InfosetEvent]) =
        this(
          ...
          pstate.currentNode.toOption.map(node => ElementName(node.asElement.name)),
